...
description: "A two-day prototype exploring generative UI patterns on DS-V2, Vanguard's framework-agnostic design system, without ever asking an LLM to write production code."
...
<aside class="callout">
  ...
  The goal: show what DS-V2 could do as a substrate for generative UI. The constraint: at the time, AI-generated code was too risky for our production environments. The solution was a two-stage LLM pipeline where every model output was constrained, structured JSON, never code. A Svelte app rendered the result deterministically. The LLM composes; the application code executes.
</aside>
...
If yes, the LLM stays in its safe lane (content generation, structured selection) while the application code keeps the deterministic guarantees that production needs. DS-V2, built as framework-agnostic Web Components with well-defined props, was the right substrate. Each component is a known entity with a constrained interface; the LLM just needed to pick which ones to use and what to fill them with.
...
The LLM was given a tool schema describing DS-V2 components and their props. Its job: generate the content that answers the prompt, AND choose the DS-V2 components best suited to display that content. The output was a JSON array of component definitions, each filled with the right property values.

What it never returned: code. Never JSX, never HTML, never CSS. The LLM didn't need to know how DS-V2 components were implemented; it just needed to know what they did and what props they took. The tool schema enforced this.
...
const componentTools = [
  ...
  // ...one tool per DS-V2 component (chart, table, list, text, accordion, ...)
];
```
...
A Svelte template walked the layout JSON, rendered each DS-V2 component with its filled props, and placed them in the specified grid structure. This step was regular application code. No LLM involvement. Predictable, testable, debuggable.
...
- **Provable design system conformance.** Only DS-V2 components can be chosen.

## What this validated

The exercise proved something I'd been betting on for years: DS-V2's framework-agnostic primitive design is durable against paradigm shifts. The library was designed before generative AI was mainstream, but it was designed right, components as standalone primitives with constrained interfaces, not framework-coupled abstractions. That made it the perfect substrate for Gen UI work: the LLM could reason about components without knowing anything about how they were built.

If DS-V2 had been a typical framework-bound component library, this prototype would have required more glue, more compromise, more code generation. Instead, it worked because the design system was already AI-friendly. The MCP server my team later built for DS-V2 under my direction extends the same idea: expose component metadata so AI agents can compose with DS-V2 without needing to learn DS-V2's internals.
...
- **Limited component coverage.** The tool schema described a subset of DS-V2; a full system would need automated schema generation from the library.
...
The same pattern generalizes beyond DS-V2. Any design system with structured component metadata and constrained props can be a Gen UI substrate. The MCP server work makes this practical: an AI coding agent can read the component schema directly, then either generate static UIs (this prototype's pattern) or scaffold component-based application code.
