...
In our case the design system is Meridian, the CMS is Adobe Experience Manager, and "consumes" means the components ship as standards-based custom elements that AEM templates declare directly.

```html
<!-- An AEM template -->
<ds-card heading="${heading}" eyebrow="${eyebrow}">
  <ds-rich-text>${body}</ds-rich-text>
</ds-card>
```
...
```text
1. ds-package, the design system. Ships custom elements + tokens.
2. ds-cms-bundle, an AEM package that vendors ds-package as a dependency.
                       Provides AEM components whose templates are just thin
                       wrappers around the custom element tags.
3. ds.css + ds.js, loaded into AEM-rendered pages via the same delivery
                       infrastructure as any other page asset.
```

The AEM component (in HTL) for the card custom element is one line of structural markup. All it does is take the dialog properties and emit:
...
**Cross-channel consistency by construction.** The same the card custom element renders identically in an Angular app, a Vue app, and an AEM-published marketing page. There's no "marketing version" of the brand that drifts from the app version.

**Brand updates that propagate.** Update a token in the design system, ship a new version of the design system package, redeploy. Every AEM page picks it up because the components are vendored, not reimplemented. The CMS doesn't need to know anything changed.
...
**A clear ownership boundary.** Meridian owns the component. AEM owns the authoring experience and the slot content. Neither side touches the other's surface. Decisions about styling go to Meridian; decisions about authoring ergonomics go to AEM. Mixing those produces fragmentation in under a quarter.
