...
<aside class="callout">
  ...
  <p>
    I led Expert Insights as a scalable AI capability, defining a
...
<div
  class="diagram"
  role="img"
  aria-label="Expert Insights architecture. Three input streams (portfolio data, methodology context, structured prompts) feed a foundational LLM, which feeds a scalable platform API, which feeds downstream surfaces. A separate LLM-as-a-judge loop scores model output and refines the prompts."
>
  <div class="diagram-row-label">inputs</div>
  <div class="diagram-row">
    <div class="diagram-node">
      <div class="diagram-node-title">portfolio data</div>
      <div class="diagram-node-body">structured account context</div>
    </div>
    <div class="diagram-node">
      <div class="diagram-node-title">methodology</div>
      <div class="diagram-node-body">guardrails, definitions, roles</div>
    </div>
    <div class="diagram-node">
      <div class="diagram-node-title">prompts</div>
      <div class="diagram-node-body">"what / so what / now what"</div>
    </div>
  </div>
  <div class="diagram-fanlines">
    <span>&#9660;</span><span>&#9660;</span><span>&#9660;</span>
  </div>

  <div class="diagram-node is-stage">
    <div class="diagram-node-title">layer 1, foundational LLM</div>
    <div class="diagram-node-body">model selection, evals, judges</div>
  </div>
  <div class="diagram-arrow">&#9660;</div>
  <div class="diagram-node is-stage">
    <div class="diagram-node-title">layer 2, scalable platform API</div>
    <div class="diagram-node-body">streaming, versioning, observability</div>
  </div>
  <div class="diagram-arrow">&#9660;</div>
  <div class="diagram-node is-stage">
    <div class="diagram-node-title">layer 3, guardrails + methodology</div>
    <div class="diagram-node-body">embedded in prompt; reinforced via judge</div>
  </div>
  <div class="diagram-fanlines">
    <span>&#9660;</span><span>&#9660;</span><span>&#9660;</span>
  </div>
  <div class="diagram-row-label">surfaces</div>
  <div class="diagram-row">
    <div class="diagram-node">
      <div class="diagram-node-title">dashboards</div>
      <div class="diagram-node-body">internal tooling</div>
    </div>
    <div class="diagram-node">
      <div class="diagram-node-title">advisor app</div>
      <div class="diagram-node-body">streaming insight UI</div>
    </div>
    <div class="diagram-node">
      <div class="diagram-node-title">downstream tools</div>
      <div class="diagram-node-body">integrations</div>
    </div>
  </div>

  <p class="diagram-note">
    A separate LLM-as-a-judge loop scores model output against the
    methodology and feeds refinements back into the prompts.
  </p>
</div>
...
- **Pattern standardization → scalable experience.** The "what / so what / now what" structure does the same job for AI output that DS-V2 component schemas do for UI, it makes the surface predictable.
- **Platform-first design.** Foundational LLM + scalable API + prompt-embedded methodology. Same instinct as framework-agnostic DS-V2: design for the substrate, not the moment.
- **Governance as product requirement.** Same posture as DS-V2 governance (Chapter 5): boundaries are not friction; they are what make the system durable.
- **Executive-ready storytelling.** The same translation work that turned DS-V2 into a business platform turned Expert Insights into something a board could endorse.

## Why it matters

This is the chapter where the platform discipline of the earlier years compounds. DS-V2 was the substrate. Expert Insights is the proof that the substrate scales into AI.
